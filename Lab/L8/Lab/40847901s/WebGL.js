...
`;
//distance( colorFromMarioDImage, vec3(0.0, 0.0, 1.0) ) < 0.6 blue cloth
//distance( colorFromMarioDImage, vec3(1.0, 0.0, 0.0) ) < 0.3  red hat and cloth
//distance( colorFromHeartTexture, vec3(1.0, 1.0, 1.0) ) < 0.7 heart
var FSHADER_SOURCE = `
    ...
    void main(){
       ...
       // vec3 texColor = texture2D( u_Sampler, v_TexCoord ).rgb;
		...
		vec3 texColor0 = texture2D( u_Sampler0, v_TexCoord ).rgb;//mario
        vec3 texColor1 = texture2D( u_Sampler1, v_TexCoord ).rgb;//LV
		vec3 texColor2 = texture2D( u_Sampler2, v_TexCoord ).rgb;//heart
        vec3 texColor;
		if(distance(texColor0, vec3(0.0, 0.0, 1.0))<0.6){
			 texColor = texColor1;
		}
	    else if(distance(texColor0, vec3(1.0, 0.0, 0.0))<0.3&&distance(texColor2, vec3(1.0, 1.0, 1.0))<0.7){
